...
import 'package:parking_app/views/owner/add_parking_space_screen.dart';
...
class _OwnerDashboardScreenState extends State<OwnerDashboardScreen> {
  ...
  List<ParkingLotsModel> parkingLotsList = [];
  String someFunction(String rentalType) {
    switch (rentalType) {
      case 'hourly':
        return '按小时计费の駐車場を処理しました';
      case 'daily':
        return '按天计费の駐車場を処理しました';
      case '':
        return '未指定計费类型，请选择计费方式';
      default:
        return '未知の計費タイプ: $rentalType';
    }
  }


late final ParkingLotService parkingLotService;
...
void initState() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildFormSection(
            child: Column(
              ...
              children: [
                ...
                SizedBox(
                  height: 270,  // 固定高度，超过内容可滚动
                  child: ListView.builder(
                    itemCount: parkingLotsList.length,
                    itemBuilder: (context, index) {
                      final lot = parkingLotsList[index];
                      int available = int.tryParse(lot.availableCapacity?.toString() ?? '') ?? 0;
                      bool isAvailable = available > 0;

                      String formattedDate = '';
                      formattedDate = "${lot.startDate.year}/${lot.startDate.month.toString().padLeft(2, '0')}/${lot.startDate.day.toString().padLeft(2, '0')}";
                            ...
                            children: [
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  IconButton(
                                    icon: Icon(
                                      lot.status == "open" ? Icons.toggle_on : Icons.toggle_off,
                                      color: lot.status == "open" ? Colors.green : Colors.grey,
                                      size: 32,
                                    ),
                                    onPressed: () {
                                      // TODO: 状态切换逻辑
                                      print('切换状态: ${lot.parkingLotName}');
                                    },
                                    tooltip: lot.status == "open" ? '有効' : '無効',
                                  ),
                                ],
                              ),
                              ...
                              Text(lot.addressDetail ?? '', style: const TextStyle(fontSize: 14)),
                              const SizedBox(height: 4),

                              // 新增容量和可用容量显示
                              Text('総容量: ${lot.capacity}台, 空き: ${lot.availableCapacity ?? 0}台'),

                              const SizedBox(height: 4),

                              Text('料金: ¥${lot.charge}/時間'),
                              const SizedBox(height: 4),

                              // 开始日期显示
                              if (formattedDate.isNotEmpty)
                                Text('開始日: $formattedDate'),
              ...
              onPressed: () async {
                try {
                  ...
                  final result = await Navigator.of(context).push(
                    MaterialPageRoute(
                      builder: (context) => AddParkingSpaceScreenScreen(ownerId: ownerId),
                    ),
                  );

                  if (result == true) {
                    // 如果返回 true，说明新增成功，执行刷新逻辑
                    _fetchParkingLots(); // 你自己的刷新方法
                  }
          ...
          // Recent reservations
          _buildSectionHeader(
            "最近の利用状況", // 修改: 移除了 AppLocalizations.of(context)?.reservations ??
            Icons.book_online,
          ),
  ...
  Widget _buildFormSection({required Widget child}) {
    return Container(
      decoration: BoxDecoration(
        color: const Color.fromARGB(239, 244, 247, 242),
  ...
  //   final user = widget.authOwnerModel;
      ...
      items: [
        ...
        if (widget.isOwner)
          BottomNavigationBarItem(
            ...
            label: "利用状況", // 使用硬编码替代AppLocalizations
          )
        else
          BottomNavigationBarItem(
  ...
  Future<void> _fetchParkingLots() async {
    final ownerId = widget.ownerId; // ✅ 关键点
    print('当前Owner ID: $ownerId');
    // final service = Provider.of<ParkingLotService>(context, listen: false);
    try {
      // final lots = await service.getParkingLotsByOwnerId(ownerId);
      final List<ParkingLotsModel> lots = [
        ParkingLotsModel(
          ownerId: 'OWNER001',
          parkingLotName: '東京駅前パーキング',
          postalCode: '114-0003', 
          prefecture: '東京都', 
          city: '北区', 
          addressDetail: '東京都北区王子駅前1-2-3',
          latitude: '', 
          longitude: '',
        ...
        ParkingLotsModel(
          ownerId: 'OWNER002',
          parkingLotName: '渋谷スクランブルパーク',
          postalCode: '104-0003', 
          prefecture: '東京都', 
          city: '渋谷区', 
          addressDetail: '東京都渋谷区恵比寿南1-2-3',
          latitude: '', 
          longitude: '',
          phoneNumber: '03-1234-5678', 
          capacity: 40,
        ...
        ParkingLotsModel(
          ownerId: 'OWNER003',
          parkingLotName: '秋葉原電気街パーキング',
          postalCode: '104-0003', 
          prefecture: '東京都', 
          city: '秋葉原', 
          addressDetail: '東京都秋葉原市秋葉原1-2-3',
          latitude: '', 
          longitude: '',
          phoneNumber: '03-1234-5678', 
          capacity: 20,
